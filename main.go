...
func mountAPIRoutes(router chi.Router, apiCfg apiConfig) {
	...
	v1.Get("/users/{userID}", apiCfg.handlerGetUser)
	v1.Get("/feed_follows/{userID}", apiCfg.handlerGetFeedFollows)
	v1.Get("/posts/{userID}", apiCfg.handlerGetPostForUser)
	v1.Get("/allPosts", apiCfg.handlerGetAllPosts)
	v1.Post("/users", apiCfg.handlerCreateUser)
	v1.Post("/feeds", apiCfg.handlerCreateFeed)
	v1.Post("/feed_follows", apiCfg.handlerCreateFeedFollows)
	v1.Delete("/feed_follows/{feedFollowID}/{userID}", apiCfg.handlerDeleteFeedFollows)
	router.Mount("/v1", v1)
}
